...
class TestRegression(unittest.TestCase):
    ...
    dataset = regression.get_dataset(BODYFAT_FILE)
    def test_get_dataset(self):

        self.assertEqual(self.dataset.shape, (252, 16))

        self.assertEqual(self.dataset[0][0], 12.6)
        self.assertEqual(self.dataset[251][0], 30.7)
        self.assertEqual(self.dataset[75][7], 91.6)

    def test_print_stats(self):
        capturedOutput = io.StringIO()
        sys.stdout = capturedOutput
        regression.print_stats(self.dataset, 1)
    ...
    def test_regression(self):
        ...
        mse = regression.regression(self.dataset, cols=[2, 3], betas=[0, 0, 0])
        numpy.testing.assert_almost_equal(mse, 418.50384920634923, 7)

        mse = regression.regression(self.dataset, cols=[2, 3, 4], betas=[0, -1.1, -.2, 3])
        numpy.testing.assert_almost_equal(mse, 11859.17408611111, 7)

    def test_gradient_descent(self):

        grad_desc = regression.gradient_descent(self.dataset, cols=[2, 3], betas=[0, 0, 0])
    ...
    def test_iterate_gradient(self):
        capturedOutput = io.StringIO()
        sys.stdout = capturedOutput
        regression.iterate_gradient(self.dataset, cols=[1, 8], betas=[400, -400, 300], T=10, eta=1e-4)
    ...
    def test_compute_betas(self):
        betas = regression.compute_betas(self.dataset, [1, 2])
    ...
    def test_predict(self):
        features = [1.0708, 23]
        prediction = regression.predict(self.dataset, [1, 2], features)
        np.testing.assert_almost_equal(prediction, 12.62245862957813)

    def test_sgd(self):
        capturedOutput = io.StringIO()
        sys.stdout = capturedOutput
        regression.sgd(self.dataset, cols=[2, 3], betas=[0, 0, 0], T=5, eta=1e-6)
